...
| 2. Partition into trail-windows | `noise_filter.windowing.TrailWindower`: for each `AlarmEvent` and each `trailId` in its `trailIds[]`, assign to a coarse tumbling time bucket of width `windowSize` (Knowledge param) keyed by `(trailId, bucketIndex)`. A window finalizes on **allowed-lateness per-bucket idleness** (event-time watermark advancing `allowedLatenessBuckets` past the bucket's OWN last add AND `idleGraceSeconds` of wall-clock elapsed) with a **wall-clock backstop** / memory valve for idle/end-of-stream (**DA-3c**, superseding DA-3b); on finalize the retained alarms are **sorted by `raisedAt`** (buffer-and-sort) so out-of-order cross-partition arrival cannot fragment or misorder a window, then it is handed to the pipeline. (Flow 1; Algorithm step A) |
...
| `noise_filter.windowing.TrailWindower` | Maintains per-`(trailId, bucketIndex)` open windows keyed by `windowSize`; finalizes on **allowed-lateness per-bucket idleness** (bucket `B` closes only once the per-trail event-time high watermark has advanced ≥ `windowAllowedLatenessBuckets` past `B`'s OWN last add AND `windowIdleGraceSeconds` of wall-clock have elapsed since it — so out-of-order cross-partition siblings all land in `B` before it closes) with a **wall-clock backstop** (`windowBackstopSeconds`, also the memory valve) for idle/end-of-stream (**DA-3c**, superseding DA-3b); a hard `windowMaxOpenWindows` cap force-finalizes the least-recently-added windows (emitted, `nf_windows_force_finalized_total`, never dropped) to bound memory; on finalize the retained alarms are **sorted by `raisedAt`** (buffer-and-sort); a late alarm for an already-finalized bucket re-opens it (`nf_windows_reopened_total`) rather than starting a dropped singleton; yields the window's alarm set **with the full enriched `AlarmEvent` objects retained** (so the emitter can populate `alarms[]`). |
...
sequenceDiagram
    ...
    W->>W: bucket by trailId and windowSize, finalize on allowed-lateness idleness plus wall-clock backstop, sort by raisedAt
...
flowchart TD
    ...
    D --> E{window finalized by allowed-lateness idleness or wall-clock backstop}
...
| DA-3b Window finalization trigger | (a) wall-clock inactivity grace since last add; (b) **event-time watermark** (finalize a `(trailId, bucket)` once event time — the per-trail high watermark of incoming `raisedAt` bucket indices — has advanced past it) **plus a wall-clock backstop** for idle/end-of-stream | **(b) event-time watermark + wall-clock backstop.** The bucket index is EVENT time (`raisedAt`), so a logical window is a slice of event time; finalization must track event-time progress, not wall-clock idleness. (a) starves batch/history replay: upstream (Enrichment) legitimately releases a single logical window's members over ~15-20 s wall-clock (self-clear hold + sweep), so a short wall-clock grace (the old 5 s) finalizes the window mid-trickle into sub-`minSamples` singletons that DBSCAN drops as noise (observed live: 2042 one-alarm windows, retention stuck ~34%). (b) finalizes bucket `B` when an alarm in a strictly-later bucket (`B + 1 + windowWatermarkLagBuckets`) arrives for the trail — correct for batch (buckets close as event time advances regardless of wall-clock dribble) AND live (event time ≈ wall clock, so the next real-time alarm closes the prior bucket promptly, no added latency). A **wall-clock backstop** (`windowBackstopSeconds`, default 60 s, env-configurable, set ABOVE the max upstream release cadence so it never fires mid-trickle) flushes a genuinely-final/idle bucket that has no successor. A late alarm for an already-finalized bucket **re-opens** it (logged, `nf_windows_reopened_total`) rather than silently forming a dropped singleton. All finalization tunables (`windowWatermarkLagBuckets`, `windowBackstopSeconds`) are env-configurable operational params — no magic constant. |
| **DA-3c Window finalization under out-of-order arrival (SUPERSEDES DA-3b)** | (a) eager event-time watermark (DA-3b — finalize `B` as soon as any alarm in a later bucket arrives); (b) **allowed-lateness / bounded-reorder: finalize a `(trailId, bucket)` only after it has been IDLE — no new member — for an allowed-lateness margin measured on BOTH the event-time watermark AND wall-clock, plus a wall-clock backstop / memory valve**; (c) global sort-then-batch of the entire corpus | **(b) allowed-lateness per-bucket idleness + buffer-sort + backstop.** DA-3b's eager watermark is WRONG under the real topic partitioning: `alarms.enriched` is keyed by `managedObjectId` by Enrichment (`kafkaTemplate.send(topic, alarm.getManagedObjectId(), json)`), and ONE trail spans many managed objects (Node, LineCard, Port, Interface, IPLink, IGPAdjacency, LSP, FiberSpan…) that hash to DIFFERENT partitions. So the alarms of ONE `(trailId, bucket)` arrive **interleaved and out of `raisedAt` order** across partitions; in P2 history replay (~24 h `raisedAt` span, ~1400 buckets in arrival-shuffled order) a forward jump on one partition closes lower buckets as singletons BEFORE their co-bucket siblings land on other partitions — the siblings then arrive "late", re-open the bucket, and re-finalize as further singletons (observed: ~3200 singleton windows, 1 cluster, trail-eligible retention **0.4%** — WORSE than before; for the busiest trail 99/115 alarms arrived below the seen max, proving genuine wide reorder, not a test artifact and not DBSCAN params — offline replay of correctly-assembled windows retains ~100%). **The fix:** a bucket is NOT finalized on watermark advance alone; it stays open until its OWN members stop arriving. `pop_finalized` closes `(trailId, B)` when EITHER (a) the trail's high watermark has advanced ≥ `windowAllowedLatenessBuckets` beyond `B`'s *own last add* AND `windowIdleGraceSeconds` of wall-clock have elapsed since that add (the watermark margin absorbs event-time skew; the wall-clock grace absorbs arrival-time skew — partition drain lag, Enrichment self-clear hold), OR (b) `windowBackstopSeconds` of wall-clock elapsed since its last add (end-of-stream / idle / memory valve). Because every batch alarm DOES eventually arrive (just out of order), each `(trailId, bucket)` collects ALL its members before closing. On finalize, alarms are **sorted by `raisedAt`** (buffer-and-sort) so arrival order never fragments or misorders the window; an out-of-order alarm whose `raisedAt` is in a still-open bucket lands there, not a fresh singleton. **Live mode stays prompt:** event time ≈ wall clock, so subsequent live alarms advance the watermark past a bucket within a bounded few `windowSize`s and the short `windowIdleGraceSeconds` (default 15 s) — finalization latency is bounded by (allowed-lateness × windowSize + idleGrace), FAR below the `windowBackstopSeconds` backstop, so no live-latency regression. **Memory bound:** the backstop always evicts any bucket `windowBackstopSeconds` after its last add, and `windowMaxOpenWindows` caps simultaneously-open windows — exceeding it force-finalizes the least-recently-added windows (emitted, `nf_windows_force_finalized_total`, never dropped), so a pathological stream cannot OOM (worst case degrades toward the old fragmentation, never grows unbounded, never silently loses an alarm). All tunables (`windowAllowedLatenessBuckets` — env `WINDOW_ALLOWED_LATENESS_BUCKETS`, default 6; `windowIdleGraceSeconds` — env `WINDOW_IDLE_GRACE_SECONDS`, default 15 s; `windowBackstopSeconds` — env `WINDOW_BACKSTOP_SECONDS`, default 300 s; `windowMaxOpenWindows` — env `WINDOW_MAX_OPEN_WINDOWS`, default 200k) are env-configurable — no magic constant. The DA-3b `WINDOW_WATERMARK_LAG_BUCKETS` env is retained as a back-compat alias onto the allowed-lateness margin. Global sort-then-batch (c) rejected: it requires bounding the whole corpus in memory before any emit (defeats streaming, no live mode). Tests: `test_windowing_out_of_order.py` feeds a same-bucket cascade INTERLEAVED with forward-jumping watermarks + other trails and asserts ONE grouped window (with an eager-watermark reference model as the negative control that FRAGMENTS the same input), a P2-style wide-reorder shuffled batch where every `(trail,bucket)` finalizes WHOLE (near-zero reopen/singleton), the buffer-sort ordering, the memory-bound force-finalize (never drops), and bounded live-mode latency. |
...
`test_out_of_order_same_bucket_cascade_stays_one_window` (DA-3c: a same-bucket cascade fed
interleaved with forward-jumping watermarks + other trails stays ONE window; carries an
eager-watermark reference model as the negative control that FRAGMENTS the same input),
`test_out_of_order_cascade_clusters_together_end_to_end` (DA-3c: the out-of-order cascade clusters
as ONE TransactionEvent end-to-end, not DBSCAN-noise singletons),
`test_wide_reorder_batch_each_group_finalizes_whole` (DA-3c: a P2-style wide-reorder shuffled batch
across many `(trail,bucket)` groups finalizes every group WHOLE, near-zero reopen/singleton; eager
model over-fragments the same shuffled input),
`test_finalized_window_alarms_are_event_time_sorted` (DA-3c buffer-and-sort),
`test_memory_bound_force_finalizes_oldest_windows_never_drops` (DA-3c `windowMaxOpenWindows` memory
bound force-finalizes oldest, emits, never drops),
`test_live_mode_finalizes_within_bound_no_excess_latency` (DA-3c: live latency stays bounded by
allowed-lateness × windowSize + idle-grace, far below the backstop),
`test_allowed_lateness_defaults_and_env_override` + `test_watermark_lag_env_alias_maps_to_allowed_lateness`
(DA-3c tunables default sanely, env-overridable, DA-3b alias retained),
...
| `WINDOW_ALLOWED_LATENESS_BUCKETS` | DA-3c reorder tolerance in whole event-time buckets (default 6). Alias `WINDOW_WATERMARK_LAG_BUCKETS` (DA-3b name) retained for back-compat. |
| `WINDOW_IDLE_GRACE_SECONDS` | DA-3c wall-clock grace since a bucket's last add before the allowed-lateness path finalizes it (default 15 s) |
| `WINDOW_BACKSTOP_SECONDS` | DA-3c wall-clock backstop / memory valve for idle / end-of-stream buckets (default 300 s; ABOVE the max upstream release cadence + reorder window) |
| `WINDOW_MAX_OPEN_WINDOWS` | DA-3c memory bound: max simultaneously-open `(trailId,bucket)` windows before least-recently-added are force-finalized (default 200000) |
...
loaded at startup and hot-refreshed on `knowledge.updated`. Dedupe-TTL, the DA-3c windowing
tunables (allowed-lateness / idle-grace / backstop / max-open — operational reorder-tolerance
params, not correlation thresholds) and read-API `limit` defaults default sanely and are
env-overridable.
...
- `GET /metrics` — Prometheus. Collectors: `nf_alarms_consumed_total`, `nf_windows_finalized_total`,
  `nf_windows_reopened_total` (late-alarm re-open of an already-finalized `(trailId,bucket)`, DA-3c),
  `nf_windows_force_finalized_total` (windows force-finalized by the `windowMaxOpenWindows` memory bound, DA-3c),
