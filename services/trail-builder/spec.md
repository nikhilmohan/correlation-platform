...
2. **Fetch graph closures from the Topology Service.** Using the Topology Service's
...
3. **Fetch trail policy from the Knowledge Service.** Retrieve the current trail
...
4. **Compute trails.** Apply the trail policy to the fetched graph data: transitive
...
5. **Persist trail definitions.** Store each trail as its member set of
...
6. **Serve trail membership queries via API.** Expose `getTrailsForObject(managedObjectId)`
...
7. **Emit `trails.built`.** After a successful build, produce a `trails.built` event
...
- **Consumes (Kafka):** `topology.changed` (payload: `TopologyChangedEvent` — fields
  `snapshotId`, `changeType`, `nodes`, `edges`; envelope `eventId` is the idempotency
  key). Python/Pydantic binding from `acp_event_model`.
...
- **APIs consumed from other services** (each is a named config-switchable integration
  ...
  - **Knowledge Service trail-policy API** — reads the current trail policy (IGP-area
    bound, SRLG-union rule, and any authored policy parameters). Used at build time.
...
8. **Config-switchable integration points.** With `TOPOLOGY_SERVICE_MODE=mock` and
...
9. **No hard-coded URLs or policy values.** Changing `TOPOLOGY_SERVICE_BASE_URL` or
   `KNOWLEDGE_SERVICE_BASE_URL` in the environment causes the service to contact the
   new address without a code change.

10. **Poison message handling.** A `topology.changed` message that cannot be
...
11. **`snapshotId` alignment.** Every trail record returned by `getTrail(trailId)`
...
12. **OpenAPI contract compliance.** Requests and responses for `GET /trails`,
...
1. **Topology Service query API surface for trail computation.** The Topology Service
   query API must expose bounded-traversal and neighbor endpoints sufficient for
   computing transitive closures (per §6.2). The exact endpoint names, path parameters,
   and response shapes are defined in the Topology Service's published `openapi.json`
   — which does not yet exist as a checked-in contract. Trail Builder cannot generate
   its Topology integration-point mock until that spec is frozen. **Blocked on:** the
   Topology Service's `openapi.json` being checked in and human-approved.
   _(Tracked: [#24](https://github.com/nikhilmohan/correlation-platform/issues/24))_

2. **Knowledge Service trail-policy API shape.** The trail policy fields (IGP-area
   boundary definition, SRLG-union rule, and any future authored parameters) that Trail
   Builder reads from the Knowledge Service are not yet specified in the Knowledge
   Service's published `openapi.json`. Trail Builder cannot build its Knowledge
   integration-point mock until this contract is frozen. **Blocked on:** the Knowledge
   Service's trail-policy API being checked in and human-approved.
   _(Tracked: [#25](https://github.com/nikhilmohan/correlation-platform/issues/25))_

3. **On-demand rebuild API authentication / authorization.** `POST /trails/rebuild`
   initiates a potentially expensive operation. Whether this endpoint requires
   authentication or is restricted to internal callers is not specified in the Solution
   Design. A human decision is needed before the designer specifies the security model.
   _(Tracked: [#27](https://github.com/nikhilmohan/correlation-platform/issues/27))_

4. **Trail record retention policy for old snapshots.** The spec states that a rebuild
   for a new `snapshotId` supersedes prior trails for that snapshot; it does not define
   how long trail records for older snapshots are retained (e.g., keep N previous
   snapshots, TTL-based expiry, or explicit deletion). The retention policy affects the
   schema design and storage sizing. **Needs human decision.**
