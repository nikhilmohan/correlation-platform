...
**In scope:**
- Consuming `patterns.approved` (`PatternApprovedEvent`) at startup and on every new approval event to maintain an in-memory (or local-state) approved-pattern set. Each approved pattern is scoped to one or more `trailId`s and carries its own per-pattern session-window rule (the **`sessionWindow`** field on `PatternApprovedEvent` — `{windowMs, type: gap-based|fixed}`, authored by the Pattern Manager; distinct from the descriptive `timing` statistics). The local pattern set is the reference for deciding which patterns are active on a given trail and what session-window governs each pattern's instances.
...
- **Per-pattern session windows:** each correlation instance's session window is governed by the **`sessionWindow`** rule carried in the pattern's own `PatternApprovedEvent` (`{windowMs, type: gap-based|fixed}`). Different patterns may carry different session-window durations/types; the engine applies each pattern's own window to its instances. `gap-based` means the window resets/extends on each new matching alarm (an idle gap of `windowMs` closes it); `fixed` means a fixed `windowMs` from instance start. A single global session-gap is not used. Other match rules (sequence, partial-match tolerance, etc.) likewise come from the pattern or from Knowledge-sourced parameters — no hard-coded thresholds anywhere.
...
1. **Load approved patterns:** on startup, fetch all currently approved patterns from the Pattern Manager API; on each `patterns.approved` event, add or refresh the pattern in the local approved-pattern set. Record which trails each pattern is active on and the pattern's per-pattern session-window rule (from the `sessionWindow` field). This local set drives which (trailId, patternId) instances may be created and what session-window governs each one.

2. **Load the codebook:** on each `codebook.generated` event, record the new `codebookId` and fetch full per-trail scenario signatures from the Codebook Generator's **`GET /codebooks/{codebookId}/trail-signatures` projection endpoint** — per scenario, fanned out per `trailId`: `{ trailId, scenarioId, rootCauseAlarmType (an alarmType-vocabulary token), expectedSymptoms: [{alarmType, managedObjectId}] }`. Always maintain the **latest codebook in scope** for each `snapshotId`/trail; when a newer `codebook.generated` event arrives for the same scope, replace the prior codebook. The fetched signatures are the reference for codebook decoding; the decode scores live alarms' `alarmType` tokens against `expectedSymptoms[].alarmType` (same vocabulary as `AlarmEvent.alarmType`); version alignment is by `snapshotId` + trail scope. (Resolves Open Question 4 — the Codebook Generator publishes this projection shape in its `openapi.json`.)
...
4. **Manage correlation-instance lifecycle (lazy init, in-progress, full-match, session-expiry):** for each (trailId, patternId) pair where the pattern is active on the trail:
   ...
   - If the instance's per-pattern session window (from the pattern's `sessionWindow` field) expires before a satisfying match: destroy the instance, emit no incident, and fire `AlarmStatusChange(reverted-open)` for each alarm that had been accumulated in the instance.

5. **Evaluate codebook decoding (fallback for unmatched alarm sets):** for an alarm set on a trail that has no active pattern instance capable of covering it, score the alarm set's `alarmType` tokens against all trail-scoped codebook scenarios' `expectedSymptoms[].alarmType` (the trail-signatures projection) using closest-match; apply scoring threshold floors sourced from the Knowledge Service; select the best-scoring scenario as the codebook decode candidate if one meets the threshold; resolve its root cause from the scenario's `rootCauseAlarmType` token; pass this candidate to conflict resolution (Task 6). (The exact interaction between concurrent pattern instances and codebook decode for the same alarm set is a design-stage decision — see Open questions.)
...
- **Consumes (Kafka):**
  ...
  - `patterns.approved` — `PatternApprovedEvent` (`patternId`, `sequence[]`, `rootCauseAlarmType`, `support`, `confidence`, `lift`, `timing`, `sessionWindow`, `codebookMatchId?`, `lifecycle`). The **`sessionWindow`** field (`{windowMs, type: gap-based|fixed}`) carries the pattern's own session-window rule that governs each correlation instance for this pattern; the `timing` field remains descriptive mining statistics. No `codebookId` field is present or required on this event; the engine resolves codebook version by `snapshotId`/trail scope from the latest `codebook.generated` event.
...
- **Produces (Kafka):**
  - `correlation.results` — `CorrelationResultEvent` (`incidentId`, `rootCauseAlarmId`, `childAlarmIds[]`, `matchedPatternId?`, `matchedCodebookId?`, `confidence`, `trailId`); one event per incident created; unchanged from prior contract (no schema change). **`matchedCodebookId` semantics (clarification — P3-G4):** on a codebook-decode incident the engine sets `matchedCodebookId` to the **`codebookId` artifact id** (from `CodebookGeneratedEvent`) of the active codebook used by the decode — **not** a scenario id and **not** the matched pattern's `codebookMatchId` (which is a codebook **scenario** reference used during Pattern Manager reconciliation). On a pattern-match incident `matchedCodebookId` is null. This is a description-level clarification; the frozen schema already states "references codebookId from CodebookGeneratedEvent" — no schema change.
...
- **APIs exposed** (published as OpenAPI 3.1 at `/openapi.json`; checked in to `services/correlation-engine/openapi.json`):
  - `GET /incidents` — list incidents; supports filter by `trailId`, time range, and match type (`pattern` | `codebook`); returns the **canonical platform list-pagination envelope** `{ items, total, limit, offset }` (the same key set as Pattern Manager's `PatternPage` and web-ui's `RunStatsPage` — frozen here for CE; see Non-functional), where each `items[]` element carries `incidentId`, `rootCauseAlarmId`, `childAlarmIds[]`, `matchedPatternId?`, `matchedCodebookId?`, `confidence`, `trailId`, `createdAt`. Query params `limit` (default 50, max 500) and `offset` (default 0).
...
- **APIs/data consumed from other services:**
  - **Pattern Manager** — `GET /patterns?lifecycle=approved` (list of all approved patterns with `patternId`, `sequence[]`, `rootCauseAlarmType`, `trailId`, `confidence`, `timing`, `sessionWindow`, `codebookMatchId?`); the `sessionWindow` field is required to obtain each pattern's per-pattern session-window rule; built against Pattern Manager's published OpenAPI.
  - **Codebook Generator** — `GET /codebooks/{codebookId}/trail-signatures?trailId={trailId}` (the CE-oriented projection): returns per scenario, fanned out per `trailId`, a `TrailScenarioSignature` `{ trailId, scenarioId, rootCauseAlarmType (alarmType-vocab token), expectedSymptoms: [{alarmType, managedObjectId}] }`. The engine consumes `expectedSymptoms` (the CE-facing projection of the scenario's predicted symptoms) and `rootCauseAlarmType`, keyed per `trailId`; built against the Codebook Generator's published `openapi.json` (resolves Open Question 4).
  - **Knowledge Service** — fetch partial-match tolerance, scoring threshold floors, and conflict-resolution parameters; all sourced from Knowledge Service — no hard-coded thresholds anywhere in the engine; built against Knowledge Service's published OpenAPI. Note: the per-pattern session-window duration/type is sourced from each pattern's `sessionWindow` field (via Pattern Manager), not from the Knowledge Service; the Knowledge Service supplies match-quality and conflict-resolution parameters.
...
- **List-pagination envelope (canonical — P3-G3):** `GET /incidents` returns the platform-canonical list envelope **`{ items, total, limit, offset }`** — the same key set as Pattern Manager's `PatternPage` and web-ui's `RunStatsPage`, so every read-API list on the platform shares one envelope and consumers read `.items` uniformly. (Alarm Manager's `GET /alarms` currently returns a divergent `{ items, page, size, totalElements, totalPages }` envelope and must adopt this same canonical shape in the alarm-manager/web-ui step to close P3-G3 end-to-end; here it is frozen for CE only.) This is an API-surface freeze of a previously design-stage detail; no event-model schema change.
...
7. **Per-pattern session windows are independent:** given two patterns P1 and P2 active on the same trail, where P1's `sessionWindow.windowMs` is W1 and P2's `sessionWindow.windowMs` is W2 (W1 not equal to W2), instance(T, P1) expires at W1 and instance(T, P2) expires at W2; neither instance's expiry is governed by the other's window duration.
...
9. **Codebook cold-start — closest-match decode without an active pattern instance:** given a set of live alarms on a trail that matches a codebook scenario (fetched via `GET /codebooks/{codebookId}/trail-signatures`) but for which no active pattern instance exists, the service scores the live alarms' `alarmType` tokens against `expectedSymptoms[].alarmType` and creates an incident with `matchedCodebookId` set to the **active codebook's `codebookId`** (the artifact id, not a scenarioId), `matchedPatternId` absent (null), and `rootCauseAlarmId` correctly resolved from the codebook scenario's `rootCauseAlarmType` token.
...
15. **Required fields populated — codebook match:** for an incident created from a codebook decode (no pattern instance), `matchedCodebookId` is non-null **and equal to the active codebook's `codebookId`** (artifact id), `matchedPatternId` is null, `confidence` is in [0, 1], and `trailId` matches the codebook scenario's trail tag.
...
23. **Codebook client fetches the trail-signatures projection and decodes (P1-G5 / P3-G2):** on a `codebook.generated` event, the engine fetches `GET /codebooks/{codebookId}/trail-signatures` (built against the Codebook Generator's published `openapi.json`) and parses per-trail `TrailScenarioSignature` `{ trailId, scenarioId, rootCauseAlarmType, expectedSymptoms: [{alarmType, managedObjectId}] }`; a codebook decode scores the live alarm set's `alarmType` tokens against `expectedSymptoms[].alarmType` (same vocabulary as `AlarmEvent.alarmType`) and resolves root cause from the scenario's `rootCauseAlarmType` token.

24. **`GET /incidents` returns the canonical pagination envelope (P3-G3):** `GET /incidents` returns a JSON object `{ items, total, limit, offset }` (not a bare array and not `{page, size, total}`); `total` is the filtered count and `limit`/`offset` are echoed; the body validates against the `IncidentPage` schema in the checked-in `openapi.json`.

25. **`matchedCodebookId` carries the codebook artifact id on a codebook-decode incident (P3-G4):** for an incident formed by a codebook decode, `CorrelationResultEvent.matchedCodebookId` equals the **`codebookId`** of the active codebook used (the `CodebookGeneratedEvent` artifact id) — not a scenarioId and not the matched pattern's `codebookMatchId`; for a pattern-match incident `matchedCodebookId` is null. (No schema change — the frozen schema description already states this reference.)
...
3. **How the pattern's session-window is expressed — RESOLVED (contract change merged).** `PatternApprovedEvent` (and `PatternDiscoveredEvent`) now carry an explicit **`sessionWindow`** field — `{windowMs (integer), type (enum: gap-based | fixed)}` — authored by the Pattern Manager and distinct from the descriptive `timing` statistics (merged event-model contract). The correlation engine reads `sessionWindow` directly: `windowMs` is the per-instance window duration; `type = gap-based` resets/extends the window on each new matching alarm (idle gap closes it), `type = fixed` is a fixed duration from instance start. What "session expires" means for a partially matched sequence (destroy + revert-open per the lifecycle above) is defined in Scope/Tasks. No further contract change is required. (Remaining design-stage detail: how the Pattern Manager *derives* the `windowMs`/`type` value from the mined `timing` — owned by the Pattern Manager design, not this service.)

4. **Codebook scenario signature fetch endpoint — RESOLVED (producer contract published).** `CodebookGeneratedEvent` carries only `codebookId`, `snapshotId`, and `scenarioCount` (summary). The engine fetches full per-trail scenario signatures via the Codebook Generator's **`GET /codebooks/{codebookId}/trail-signatures?trailId={trailId}` projection endpoint**, which returns per scenario, fanned out per `trailId`, a `TrailScenarioSignature` `{ trailId, scenarioId, rootCauseAlarmType (an alarmType-vocab token), expectedSymptoms: [{alarmType, managedObjectId}] }`. This is exactly the per-`trailId` `{trailId, rootCauseAlarmType, expectedSymptoms[]}` shape the engine needs; the Codebook Generator published it in its `openapi.json` (the producer-side fix for data-integration gaps P1-G5 / P3-G2). The engine builds its HTTP client and mock stub against that published OpenAPI, consuming `expectedSymptoms` (the CE-facing projection alias) and `rootCauseAlarmType`. No contract change on the CE side — `CorrelationResultEvent` is unchanged.
