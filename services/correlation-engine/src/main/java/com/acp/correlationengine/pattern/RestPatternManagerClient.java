...
public class RestPatternManagerClient implements PatternManagerClient {
    ...
    public List<PatternRef> listApproved() {
        return PatternViewMapper.fromPage(fetchApprovedPage());
    }

    @Override
    public Optional<String> discoverSnapshotId() {
        return PatternViewMapper.snapshotIdFromPage(fetchApprovedPage());
    }

    private JsonNode fetchApprovedPage() {
        return http.get()
