...
- **Consumes (Kafka):**
  - `patterns.mined` — `PatternMinedEvent` (Pattern Miner output: `sequence[]`, `support`, `confidence`, `lift`, `trailId`, `timing`, `provenance{sourceWindowId, snapshotId, codebookVersion}`). The `sequence[]` tokens are `alarmType`-vocabulary tokens (the canonical join key). The `timing` field carries the inter-arrival / timeframe statistics the Pattern Manager uses as the sole input for session-window derivation; the derivation reads the pinned ms sub-keys `timeframeMs` + `medianInterArrivalMs` (and optional `maxInterArrivalMs` + `stddevInterArrivalMs`). `timing` is an open object in the contract, so these keys are a cross-service contract-of-shape the Pattern Miner must populate (Pattern-Miner follow-up; bridged meanwhile by a documented alias/unit-normalisation map — P2-GAP-05). No event-model change.

- **Produces (Kafka):**
  - `patterns.discovered` — `PatternDiscoveredEvent` (one per draft pattern persisted: `patternId`, `sequence[]`, `rootCauseAlarmType`, `support`, `confidence`, `lift`, `timing`, `sessionWindow` ({`windowMs`: integer ms, `type`: `gap-based`|`fixed`}), `codebookMatchId?`, `lifecycle = draft`). `rootCauseAlarmType` is an **`alarmType`-vocabulary token** (Knowledge `alarmTypeVocabulary`; same value space as `AlarmEvent.alarmType` and codebook `predictedSymptoms[].alarmType` — P2-GAP-04). `sessionWindow` is **required** on every emitted event per the merged `libs/event-model` contract — the Pattern Manager derives and populates it from the mined timing.
  - `patterns.approved` — `PatternApprovedEvent` (one per approval transition: `patternId`, `sequence[]`, `rootCauseAlarmType`, `support`, `confidence`, `lift`, `timing`, `sessionWindow` ({`windowMs`: integer ms, `type`: `gap-based`|`fixed`}), `codebookMatchId?`, `lifecycle = approved`). `rootCauseAlarmType` is an **`alarmType`-vocabulary token** (P2-GAP-04). Note: this event carries **no `trailId`** (and none is added — the Correlation Engine reads `trailId` from the `GET /patterns` read API, P3-G1). `sessionWindow` is **required** on every emitted event per the merged `libs/event-model` contract — carried from the Pattern Store's persisted value for the pattern. The Pattern Manager is the sole producer; the web-ui signals approval only via the Pattern Manager API (see APIs exposed below), never by publishing to this topic directly. Architecture.md's "web-ui produces patterns.approved (via API)" means the web-ui drives approval through the Pattern Manager API, and the Pattern Manager emits the event.
...
- **APIs exposed** (published as OpenAPI 3.1 at `/openapi.json`; `openapi.json` checked in to `services/pattern-manager/openapi.json`):
  - `GET /patterns` — list patterns; supports filter by `lifecycle` (`draft`, `approved`, `deprecated`) plus offset pagination (`limit`/`offset`) and `sort`. Returns the **`PatternPage` envelope** `{ items: PatternView[], total, limit, offset }` (a JSON object envelope — **not** a bare array; frozen as SSoT in `openapi.json` — P2-GAP-08). Each `PatternView` item carries: `patternId`, **`trailId`** (the trail the pattern was mined from, from `PatternMinedEvent.trailId` — the Correlation Engine's per-pattern `trailId` source for `(trailId, patternId)` keying, since the frozen `PatternApprovedEvent` carries none — P3-G1), `sequence[]`, `rootCauseAlarmType` (an `alarmType`-vocabulary token — P2-GAP-04), `support`, `confidence`, `lift`, `timing`, `sessionWindow` ({`windowMs`, `type`}), `codebookMatchId`, `instanceCount`, `supportingInstances[]`, `structurallyValidated`, `structuralValidationReason`, `lifecycle`.
  - `GET /patterns/{patternId}` — retrieve a single pattern by `patternId` as a full `PatternView` including `trailId`, `supportingInstances[]`, `structurallyValidated`, `structuralValidationReason`, and `sessionWindow` ({`windowMs`, `type`}) in the response and XAI metadata.
  ...
  - `PATCH /patterns/{patternId}` — **operator edit of a draft pattern (placeholder; to be enhanced).** Accepts the **frozen edit body** `PatternEdit { sequenceFlags: [{ index: integer, optional: boolean }], reviewer, notes? }` (per-position `optional` markers on the sequence; frozen as SSoT in `openapi.json` — P2-GAP-06). Allowed only while the pattern is in `draft` (reject 409/422 otherwise; out-of-range `index` is 422). Persist the edits onto the draft pattern record in the Pattern Store and return the updated record. The edit metadata (e.g. `optional` markers) is **internal to the Pattern Store + read API for now — NOT added to the frozen `PatternApprovedEvent`**; how an edited/optional-alarm pattern is represented to and matched by the Correlation Engine is a **documented post-MVP / design-stage enhancement** (the Correlation Engine already tolerates missing alarms via its partial-match tolerance). The `sessionWindow` is derived and read-only in MVP — it is not an editable field via this endpoint.
