...
| **`schemaVersion` semantics** | (a) integer major only (accept 1, reject ≥2); (b) `"major.minor"` string | **(a) integer.** The spec defines `schemaVersion` as an integer with major-version accept/reject; minor is additive and not encoded separately for MVP. |
...
| 14 | TransactionEvent required fields | **Java** `TransactionEventTest.missing_<field>_rejected()` ×6 + `validAccepted()`; **Python** `test_transaction.py::test_missing_field_rejected[<field>]` ×6 + `test_valid` | missing any of `transactionId,trailId,snapshotId,alarmIds,windowStart,windowEnd` raises; full event accepted |
